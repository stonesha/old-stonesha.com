...
export const getMangaInfo = async (tag) => {
    ...
    const updated = moment(parsed_response.querySelector(".stre-value").structuredText.replace(" - ", " ").replace(",", " ").slice(0, -3)).format("YYYY-MM-DD");
...
export const getFollowedMangas = async () => {
    ...
    const mangaList = await Promise.all(mangas.map(async (manga) => { return await getMangaInfo(manga); }))
        .then((mangaList) => mangaList.sort((a, b) => b.updated - a.updated));
